...
def find_audio_files(
    root_dir: str,
    allowed_ext: Union[str, List[str]],
    skip_dir: str = None,
...
class AudioDataset(Dataset):
    ...
    def get_random_sample_duration(self) -> int:
        duration = 1
        if 'clip_duration' in self.cfg.trainer:
            duration = self.cfg.trainer.clip_duration
        if self.subset_mode == 'train':
    ...
    def get_random_clips(self, n: int) -> Tuple[Optional[AudioClip], Optional[AudioClip], Optional[str]]:
        ...
        if not lq_clip:
            ...
            if self.cfg.dataset.fallback_generated_eqs:
                ...
                if not self.cfg.dataset.ignore_lq_clips:
    ...
    def __getitem__(self, n: int) -> Tuple[Optional[Tensor], Tensor, str]:
        lq_clip, hq_clip, eq_name = self.get_random_clips(n)
        if not hq_clip:
            raise RuntimeError(f'Invalid HQ clip for index {n}')
        if not lq_clip:
            if self.cfg.dataset.ignore_lq_clips:
                return None, hq_clip.waveform, eq_name
            else:
...
class AudioDatasetExtended(AudioDataset):

    def __getitem__(self, n: int) -> Tensor:
        lq_waveform, hq_waveform, eq_name = super().__getitem__(n)

        if self.cfg.trainer.audio_mode == 'melspectrogram':
            # TODO: might be improved because the current implementation does resempling of individual samples in the loading phase
            # to match each audio track to cfg.dataset.sample_rate, then we do resampling again to match the dataset sample rate to
            # cfg.trainer.melspectrogram.sample_rate in the transform phase (see below)
            # This is not optimal, we should do the resampling only once, and then use the same sample rate for the whole dataset
            # it is kept like this for backward compatibility with the previous implementation
            if self.cfg.trainer.melspectrogram.sample_rate != self.cfg.dataset.sample_rate:
                hq_waveform, _ = signal.resample_signal(hq_waveform, self.cfg.dataset.sample_rate, self.cfg.trainer.melspectrogram.sample_rate)
                lq_waveform, _ = signal.resample_signal(lq_waveform, self.cfg.dataset.sample_rate, self.cfg.trainer.melspectrogram.sample_rate)


            # Here we do the following:
            # 1. We load the audio track
            # 2. Get random slice of fixed size from audio
            # 3. Compute melspectrogram and get image representation
            # 4. Convert it to 3 channels and float32 + normalize
            mel = Mel(
                x_res=self.cfg.trainer.melspectrogram.x_res,
                y_res=self.cfg.trainer.melspectrogram.y_res,
                hop_length=self.cfg.trainer.melspectrogram.hop_length,
                sample_rate=self.cfg.trainer.melspectrogram.sample_rate,
                n_fft=self.cfg.trainer.melspectrogram.n_fft,
                n_iter=self.cfg.trainer.melspectrogram.griffin_lim_iters,
            )
            mel.load_audio(raw_audio=hq_waveform.numpy()[0, :])

            slice_index = self.rnd.integers(0, mel.get_number_of_slices())
            hq_img = mel.audio_slice_to_image(slice_index)
            hq_img = np.expand_dims(np.array(hq_img), axis=2).astype(np.float32) / 127.5 - 1
            
            if self.cfg.dataset.ignore_lq_clips:
                return np.transpose(hq_img, (2, 0, 1)), {}

            mel.load_audio(raw_audio=lq_waveform.numpy()[0, :])
            lq_img = mel.audio_slice_to_image(slice_index)
            lq_img = np.expand_dims(np.array(lq_img), axis=2).astype(np.float32) / 127.5 - 1

            # return hq image + lq image
            return np.transpose(hq_img, (2, 0, 1)), np.transpose(lq_img, (2, 0, 1))

        elif self.cfg.trainer.audio_mode == 'raw':
            # TODO: might be improved because the current implementation does resempling of individual samples in the loading phase
            # to match each audio track to cfg.dataset.sample_rate, then we do resampling again to match the dataset sample rate to
            # cfg.trainer.melspectrogram.sample_rate in the transform phase (see below)
            # This is not optimal, we should do the resampling only once, and then use the same sample rate for the whole dataset
            ...
            hq_raw = hq_waveform[:, mel.slice_size * slice_index : mel.slice_size * (slice_index + 1)]

            if self.cfg.dataset.ignore_lq_clips:
                return hq_raw, {}

            lq_raw = lq_waveform[:, mel.slice_size * slice_index : mel.slice_size * (slice_index + 1)]

            return hq_raw, lq_raw

        elif self.cfg.trainer.audio_mode == 'wavelet':
            pass
        else:
            raise ValueError(f"Audio mode {self.cfg.trainer.audio_mode} not supported")
...
def create_generator_from_dataloader(loader: DataLoader):
    while True:
        yield from loader
